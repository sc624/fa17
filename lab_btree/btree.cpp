...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(subroot->is_leaf && subroot->elements[first_larger_idx].key != key)
      return V();
    else
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    DataPair insertmid = *mid_elem_itr;
    parent->elements.insert(elem_itr, insertmid);
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    {
      BTreeNode* newN = subroot->children[first_larger_idx];
      insert(newN, pair);
      if(newN->elements.size() >= order)
      {
